    ...
    <div>
        <div class=" widthfull flex flex-wrap py-20 justify-between">
            <div class="w-full md:ps-20 md:w-[40%] ">
                <h3 class=" font-bold  text-2xl">{{ trans('website/website.ForgotPassword') }}</h3>
                <p class=" text-gray2 fontp font-normal text-base leading-[135%] my-6 text-start">
                    {{ trans('auth.please enter the phone number you use it when you sign up') }}
                </p>
                ...
                <form action="{{ route('instructor.forgotPasswordPhone') }}" method="get">
                    @csrf
                    <div class=" w-full mt-5">
                        <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('auth.Phone Number') }}</p>
                        <input class="inputtext " placeholder="{{ trans('auth.Phone Number') }}" type="text"
                ...
                <a href="{{ route('login') }}">
                    <p class=" text-orange hover:text-orange3 underline underline-offset-4 text-sm  mt-6 font-bold ">
