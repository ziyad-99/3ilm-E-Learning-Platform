    ...
    <div>
        <div class=" widthfull flex py-20 justify-between">
            <div class="w-full  ">
                ...
                @if(Schema::hasColumn($course->getTable(), 'branch'))
                    <p class="font-bold fontp text-center mt-4 text-xl">{{ $studentGroup->courseable->year }}
                        - {{ $studentGroup->courseable->level }}</p>
                    <p class="font-medium fontp text-center mt-3 text-base">{{ $studentGroup->name }}</p>
                    <p class="font-medium fontp text-orange text-center mt-2 text-base">{{ $studentGroup->courseable->branch }}</p>
                @else
                    <p class="font-medium fontp text-center mt-3 text-base">{{ $studentGroup->name }}</p>
                    <p class="font-medium fontp text-orange text-center mt-2 text-base">{{ $studentGroup->courseable->level }}</p>
                @endif

                <div class="w-full md:w-5/12 mt-5 mx-auto px-20 text-center ">
                    <p class="text-black text-base font-medium mb-2 fontp">{{ $studentGroup->sessions->count() }}
                        /{{ $course->numberSessions }}</p>
                    <div class=" bg-[#D9D9D9] h-3 w-full rounded-full">
                        <div class="bg-orange rounded-full h-full"
                             style="width:{{ (($studentGroup->sessions->count() / $course->numberSessions) * 100) > 100 ? 100 : (($studentGroup->sessions->count() / $course->numberSessions) * 100) }}%"></div>
                    </div>
                </div>
                    ...
                    <div class="  grid grid-cols-1 gap-4 md:gap-2 md:grid-cols-3 px-6">
                        <div class=" w-full md:col-span-2">
                            <div class="h-[450px] bg-[#f5f5f5] w-full flex justify-center items-center">
                                @if($studentGroup->sessions->last()->status == 1)
                                    {{--                                    <a class=" btn items-center hover:bg-orange3 transform hover:scale-105 ms-0 mt-5 flex px-10"--}}
                                    {{--                                       href="{{ route('student.joinMeeting', ['session_id' => $studentGroup->sessions->last()->id ]) }}"><span--}}
                                    {{--                                            class=" text-white font-bold text-sm">{{ trans('website/zoom/zoomPage.join') }} {{ $studentGroup->sessions->last()->name }}</span></a>--}}


                                    <button id="myButton"
                                            class=" btn items-center hover:bg-orange3 transform hover:scale-105 ms-0 mt-5 flex px-10">
                                            <span class=" text-white font-bold text-sm">
                                                {{ trans('website/zoom/zoomPage.join') }} {{ $studentGroup->sessions->last()->name }}
                                            </span>
                                    </button>
                                    ...
                                    <script>
                                        ...
                                        function showIframe() {
                                            ...
                                            document.getElementById('myIframe').src = @json(route('student.joinMeeting', ['session_id' => $studentGroup->sessions->last()->id ]));
                                    ...
                                    {{--                                            console.log(@json(route('instructor.mycourse')));--}}
                                    {{--                                            // console.log("the src route is ", iframe.src);--}}

                                    {{--                                            let endRoute = @json(route('instructor.endMeeting', ['meetingId' => $studentGroup->sessions->last()->meetingID]));--}}
                                    {{--                                            console.log("the end route is ", endRoute);--}}

                                    {{--                                            let loadedRout = iframe.contentWindow.location.href;--}}
                                    {{--                                            console.log("the loaded route is ", loadedRout);--}}

                                    {{--                                            // Check if the iframe's src is not null or empty--}}
                                    {{--                                            if (endRoute === loadedRout) {--}}
                                    {{--                                                // // Reload the parent page when the src changes--}}
                                    {{--                                                // reloadParent();--}}
                                    {{--                                                window.top.location.reload(@json(route('instructor.mycourse')));--}}
                                    {{--                                            }--}}
                                    {{--                                        });--}}
                                    {{--                                    </script>--}}
                                @else
                                    {{--                                    <a class=" btn items-center hover:bg-orange3 transform hover:scale-105 ms-0 mt-5 flex px-10"--}}
                                    {{--                                       href="{{ route('student.recordeLink', ['meeting_id' => $studentGroup->sessions->last()->meetingID ]) }}"><span--}}
                                    {{--                                            class=" text-white font-bold text-sm">{{ trans('website/zoom/zoomPage.session_end') }}</span></a>--}}
                                    ...
                                    <script>
                                        ...
                                        function showIframe() {
                                            document.getElementById('myButton').style.display = 'none';
                                            document.getElementById('myIframe').src = @json(route('student.recordeLink', ['meeting_id' => $studentGroup->sessions->last()->meetingID ]));
                                            document.getElementById('myIframe').style.display = 'block';
                                        }
                                    </script>
                                @endif
                        ...
                        <div class=" w-fullpy-4 ps-2">
                            ...
                            @forelse($studentGroup->sessions as $session)
                        ...
                        <ul id="tabs2" class="inline-flex flex-wrap gap-2 pb-2 mt-8 px-1.5">
                            <li class=" ">
                                <a id="default-tab"
                                   ...
                                   href="#first">{{ trans('website/zoom/zoomPage.Resources') }}</a>
                            </li>
                            <li>
                                <a class="font-semibold text-[15px] mx-2 md:text-base md:mx-4 transition-all border-[#FF6900] border-solid pb-2"
                                   href="#second">{{ trans('website/zoom/zoomPage.Quiz') }}</a></li>
                            {{--                            <li>--}}
                            {{--                                <a class="font-semibold text-[15px] mx-2 md:text-base md:mx-4 transition-all border-[#FF6900] border-solid pb-2"--}}
                            {{--                                   href="#third">{{ trans('website/zoom/zoomPage.Discussion') }}</a></li>--}}
                            <li>
                                <a class="font-semibold text-[15px] mx-2 md:text-base md:mx-4 transition-all  border-[#FF6900] border-solid pb-2"
                                   href="#fourth">{{ trans('website/zoom/zoomPage.Certificate') }}</a></li>
                        </ul>
                        ...
                        <div id="tab-contents2">
                            <div id="first" class="  p-6 bg-[#f5f5f5] border rounded border-[#eee] ">
                                <div title="Resources" class="w-full">
                                    @forelse($studentGroup->sessions->last()->ressources as $ressource)
                                    ...
                                    @empty
                                        <a>
                                            <p class="fontp text-black34 hover:text-orange font-medium mb-2 text-base leading-[20px] text-start">
                                                {{ trans('website/zoom/zoomPage.no_resources') }}
                                            </p>
                                        </a>
                                    @endforelse
                                </div>
                            ...
                            @php
                                $lastSession = $studentGroup->sessions->last();
                                ...
                                <div class="min-w-[600px]">
                                    ...
                                    @empty
                                        <p class="font-normal fontp w-2/12 text-[15px]">
                                            {{ trans('website/zoom/zoomPage.no_quiz') }}
                                        </p>
                                    @endforelse
                                    ...
                                    <div title="Quiz2" class=" w-full">
                                        <div class=" w-full px-6 border-b border-[#E2E2E2] border-solid">
                                            <div class="flex">
                                                <p class="font-semibold w-3/12 text-gray2 text-sm pb-2">
                                                    {{ trans('website/zoom/zoomPage.quiz_name') }}
                                                </p>
                                                <p class="font-semibold w-2/12 text-gray2 text-sm pb-2">
                                                    {{ trans('website/zoom/zoomPage.quiz_type') }}
                                                </p>
                                                <p class="font-semibold w-2/12 text-gray2 text-sm pb-2">
                                                    {{ trans('website/zoom/zoomPage.total_questions') }}
                                                </p>
                                                <p class="font-semibold w-2/12 text-gray2 text-sm  pb-2">
                                                    {{ trans('website/zoom/zoomPage.Duration') }}
                                                </p>
                                                <p class="font-semibold ms-auto text-gray2 w-3/12 text-sm   pb-2">
                                                    {{ trans('website/zoom/zoomPage.Action') }}
                                                </p>
                                            </div>
                                        </div>
                                        @forelse($lastSession->quizs as $quiz)
                                            @if(!$quiz->marks->where('student_id', $student->id)->first())
                                                <div
                                                    ...
                                                    <p class="font-normal fontp w-2/12 text-[15px]  ">
                                                        {{--splite the duration to hours and minutes and seconds--}}
                                                        @php
                                                            $time = explode(':',$quiz->duration);
                                                        @endphp
                                                        {{ $time[0].'h '.$time[1].'m '.$time[0].'s' }}
                                                    </p>
                                        ...
                                        @empty
                                            <p class="font-normal fontp w-2/12 text-[15px]">
                                                {{ trans('website/zoom/zoomPage.no_quiz_taken') }}
                                            </p>
                                        @endforelse
                                    </div>
                            ...
                            <div id="fourth" class="hidden  p-6 bg-[#f5f5f5]">
                                <div title="Resources"
                                    ...
                                    <h3 class="text-gray2 font-bold text-center text-3xl">{{ trans('website/zoom/zoomPage.Congratulations') }}</h3>
                                    <button
                                        class=" mx-auto hover:bg-orange3 transition-all	 transform hover:scale-[1.01] btn items-center justify-center flex my-4 px-6">
                                        <span
                                            class=" text-white font-bold text-sm ">{{ trans('website/zoom/zoomPage.download_certificate') }} </span>
                                    </button>
