    ...
    <div class="bg-[#f5f5f5]">
        <div class="w-full md:ps-24 md:px-10">
            <h4 class="font-bold text-black34 mx-auto my-5 text-2xl px-4 lg:leading-[120%] ">
                <span>{{ trans('admin/admin.EditGroup') }}</span>
            </h4>
            ...
            <form id="myForm" class="w-full" action="{{ route('admin.updateGroup') }}" method="post">
                @csrf
                <div class="  w-full bg-white rounded-2xl p-4  py-10 md:px-9">
                        ...
                        <div class=" w-full md:w-7/12">
                            ...
                            <div class="w-full mt-10">
                                ...
                                <script>
                                    ...
                                    var days = @json($days ?? []);
                                    var startTimes = @json($startTimes ?? []);
                                    var endTimes = @json($endTimes ?? []);

                                    // Add event listener to each checkbox
                                    checkboxes.forEach(checkbox => {

                                        //get the day time inputs
                                        var dayTime = document.getElementById(checkbox.name + 'Time');
                                        var startTime = document.getElementById(checkbox.name + 'StartTime');
                                        var endTime = document.getElementById(checkbox.name + 'EndTime');

                                        if (days.includes(checkbox.name)) {
                                            checkbox.checked = true; // Check the checkbox

                                            // Perform actions when checkbox is checked
                                            dayTime.style.display = 'block';
                                            startTime.setAttribute('required', 'required');
                                            startTime.value = startTimes[checkbox.name + 'StartTime'];

                                            endTime.setAttribute('required', 'required');
                                            endTime.value = endTimes[checkbox.name + 'EndTime'];
                                        }
                            ...
                            <div class=" w-full mt-4">
                                ...
                                <select class="inputtext " name="courseable_type" id="courseType" required>
                                    <option disabled>{{ trans('admin/admin.SelectCourseType') }}</option>
                                    <option value="App\Models\Course\SupportingCourse"
                                        {{ $group->courseable_type === 'App\Models\Course\SupportingCourse' ? 'selected' : ''}}>
                                        {{ trans('admin/admin.SupportingCourse') }}
                                    </option>
                                    <option value="App\Models\Course\LanguageCourse"
                                        {{ $group->courseable_type === 'App\Models\Course\LanguageCourse' ? 'selected' : ''}}>
                                        {{ trans('admin/admin.LanguagesCourse') }}
                                    </option>
                                    <option value="App\Models\Course\IntensiveCourse"
                                        {{ $group->courseable_type === 'App\Models\Course\IntensiveCourse' ? 'selected' : ''}}>
                                        {{ trans('admin/admin.IntensiveCourse') }}
                                    </option>
                                </select>
                            </div>

                            <div class=" w-full mt-4" id="supportingCourses"
                                 style="{{ $group->courseable_type ==='App\Models\Course\SupportingCourse' ? 'display:block;' : 'display:none;'}}">
                                ...
                                <select class="inputtext" name="supportingCourse_id" id="supportingCourseValue">
                                    <option value=""
                                            {{ $group->courseable_type !=='App\Models\Course\SupportingCourse' ? 'selected' : ''}} disabled>
                                        {{ trans('admin/admin.SelectCourse') }}
                                    </option>
                                    @foreach($sup_courses as $course)
                                        <option value="{{ $course->id }}"
                                            {{ $group->courseable_type ==='App\Models\Course\SupportingCourse' ? $course->id === $group->courseable_id ? 'selected' : '' : ''}}>{{ $course->title }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class=" w-full mt-4" id="languagesCourses"
                                 style="{{ $group->courseable_type ==='App\Models\Course\LanguageCourse' ? 'display:block;' : 'display:none;'}}">
                                ...
                                <select class="inputtext" name="languagesCourse_id" id="languagesCourseValue">
                                    <option value=""
                                            {{ $group->courseable_type !=='App\Models\Course\LanguageCourse' ? 'selected' : ''}} disabled>
                                        {{ trans('admin/admin.SelectCourse') }}
                                    </option>
                                    @foreach($lang_courses as $course)
                                        <option value="{{ $course->id }}"
                                            {{ $group->courseable_type ==='App\Models\Course\LanguageCourse' ? $course->id === $group->courseable_id ? 'selected' : '' : ''}}>{{ $course->title }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class=" w-full mt-4" id="intensiveCourses"
                                 style="{{ $group->courseable_type ==='App\Models\Course\IntensiveCourse' ? 'display:block;' : 'display:none;'}}">
                                ...
                                <select class="inputtext " name="intensiveCourse_id" id="intensiveCourseValue">
                                    <option value=""
                                            {{ $group->courseable_type !=='App\Models\Course\IntensiveCourse' ? 'selected' : ''}} disabled>
                                        {{ trans('admin/admin.SelectCourse') }}
                                    </option>
                                    @foreach($inten_courses as $course)
                                        <option value="{{ $course->id }}"
                                            {{ $group->courseable_type ==='App\Models\Course\IntensiveCourse' ? $course->id === $group->courseable_id ? 'selected' : '' : ''}}>{{ $course->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <button type="submit"
                        ...
                        <span class=" text-white font-bold text-sm">{{ trans('admin/admin.CreateGroup') }}</span>
                    </button>
                </div>
            </form>
