...
@section('title')
    {{ trans('admin/admin.InstructorsFinancialsReport') }}
@endsection
    ...
    <div>
        <div class="w-full md:ps-24 md:px-10">
            <h4 class="font-bold text-black34 mx-auto my-5 text-2xl px-4 lg:leading-[120%] ">
                <span>{{ trans('admin/admin.InstructorsFinancialsReport') }}</span>
            </h4>
            <div class="w-full px-2 pt-5 mb-5 flex flex-wrap ">
                <div class="w-full md:px-3 md:w-4/12">
                    <div class=" flex items-center flex-col w-full bg-white rounded-2xl py-10 p-4">
                        <p class="text-3xl font-semibold text-orange fontp">{{ $totalInstructors }}</p>
                        <p class="text-lg font-semibold text-black34 fontp">{{ trans('admin/admin.TotalInstructors') }}</p>
                    </div>
                </div>
                    ...
                    <div>
                        <table class="table admin" datatable id="datatable-search-list">
                            ...
                            <tr class="bg-orange/30">
                                ...
                                <th class="font-bold uppercase text-orange text-xxs opacity-70">{{ trans('admin/admin.Courses') }}</th>
                                ...
                                <th class="font-bold uppercase text-orange text-xxs opacity-70">{{ trans('admin/admin.instructor salary') }}</th>
                            </tr>
                            </thead>
                            <tbody>
                            ...
                            @foreach ($instructors as $instructor)
                                @php
                                    ...
                                    $coursesNumber = $supportingCourses->count() + $intensiveCourses->count() + $languagesCourses->count();
                                    $studentsNumber = 0;
                                    ...
                                    $supCourseEarning = 0;
                                    $lanCourseEarning = 0;
                                    $intenCourseEarning = 0;

                                    $supCourseSessions = 0;
                                    $lanCourseSessions = 0;
                                    $intenCourseSessions = 0;
                                ...
                                @foreach ($supportingCourses as $course)
                                    @php
                                        ...
                                        $studentsNumber += $courseEnrollments;
                                        $supCourseEarning += $courseEnrollments * $course->price;

                                        if ($course->paymentType == 'percentage')
                                        {

                                            $instructorSalary = $instructorSalary + $supCourseEarning * ( $course->percentage / 100 );

                                        } else {

                                         foreach ($course->groups as $group)
                                             {
                                               $supCourseSessions += $group->sessions->count();
                                             }

                                         $instructorSalary = $instructorSalary + $supCourseSessions * $course->perSession;
                                        }
                                    @endphp
                                @endforeach
                                ...
                                @foreach ($languagesCourses as $course)
                                    @php
                                        ...
                                        $studentsNumber += $courseEnrollments;
                                        $lanCourseEarning += $courseEnrollments * $course->price;

                                        if ($course->paymentType == 'percentage')
                                        {

                                            $instructorSalary = $instructorSalary + $lanCourseEarning * ( $course->percentage / 100 );

                                        } else {

                                         foreach ($course->groups as $group)
                                             {
                                               $lanCourseSessions += $group->sessions->count();
                                             }

                                         $instructorSalary = $instructorSalary + $lanCourseSessions * $course->perSession;
                                        }
                                ...
                                @foreach ($intensiveCourses as $course)
                                    @php
                                        ...
                                        $studentsNumber += $courseEnrollments;
                                        $intenCourseEarning += $courseEnrollments * $course->price;


                                        if ($course->paymentType == 'percentage')
                                        {

                                            $instructorSalary = $instructorSalary + $intenCourseEarning * ( $course->percentage / 100 );

                                        } else {

                                         foreach ($course->groups as $group)
                                             {
                                               $intenCourseSessions += $group->sessions->count();
                                             }

                                         $instructorSalary = $instructorSalary + $intenCourseSessions * $course->perSession;
                                        }
                                ...
                                <tr>
                                    ...
                                    <td class="text-sm font-medium text-black34 fontp leading-normal">
                                        {{ $supCourseEarning + $lanCourseEarning + $intenCourseEarning }} {{ trans('admin/admin.(DA)') }}
                                    </td>
                                    <td class="text-sm font-medium text-black34 fontp leading-normal">
                                        {{ $instructorSalary }} {{ trans('admin/admin.(DA)') }}
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
