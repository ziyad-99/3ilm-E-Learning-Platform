...
@section('title')
    {{ trans('admin/admin.LanguageCoursesPanel') }}
@endsection
    ...
    <div>
        <div class="w-full md:ps-24 md:px-10">
            <h4 class="font-bold text-black34 mx-auto my-5 text-2xl  lg:leading-[120%] ">
                <span>{{ trans('admin/admin.LanguageCoursesPanel') }}</span>
            </h4>

            <form action="" method="post">
                @csrf
                <input hidden name="courseType" value="languagesCourse" required>
            ...
            @can('add_languages_courses')
                <a href="{{ route('admin.addLanguageCourse') }}"
                   class=" btn items-center mt-5 justify-center hover:bg-orange3 transition-all	 transform hover:scale-[1.01] flex px-6">
                    <span class=" text-white font-bold text-sm ">{{ trans('admin/admin.AddLanguageCourse') }}</span>
                </a>
            @endcan
        </div>
        <div>
                        ...
                        <tr>
                            ...
                            <td class="text-sm font-normal text-black34 fontp leading-normal">
                                <div class=" w-full mt-4">
                                    @can('change_status_languages_courses')
                                        <form
                                            action="{{ route('admin.activeOrDisableCourse', ['courseType' => 'langCourse', 'course_id' => $course->id]) }}"
                                            method="post">
                                            @csrf
                            ...
                            <td class="text-sm font-normal leading-normal">
                                <div class="relative text-sm group justify-end gap-2 flex font-semibold ">
                                    ...
                                    @can('delete_languages_courses')
                                        <a id="deleteCourse" href="{{ route('admin.deleteLanguageCourse', ['course_id'=>$course->id]) }}"
