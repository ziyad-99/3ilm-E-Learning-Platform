...
@section('title')
    {{ trans('admin/admin.IntensiveCoursesPanel') }}
@endsection
    ...
    <div>
        <div class="w-full md:ps-24 md:px-10">
            <h4 class="font-bold text-black34 mx-auto my-5 text-2xl  lg:leading-[120%] ">
                <span>{{ trans('admin/admin.IntensiveCoursesPanel') }}</span>
            </h4>

            <form action="" method="post">
                @csrf
                <input hidden name="courseType" value="intensiveCourse" required>
                            ...
                            <div class="md:w-8/12 flex gap-2">
                                <div class="md:w-4/12 gap-2">
                                    <select class="inputtext" name="intensiveCourseLevel"
                                            id="intensiveCourseLevels" required>
            ...
            @can('add_intensive_courses')
                <a href="{{ route('admin.addIntensiveCourse') }}"
                   class=" btn items-center mt-5 justify-center hover:bg-orange3 transition-all	 transform hover:scale-[1.01] flex px-6">
                    <span class=" text-white font-bold text-sm ">{{ trans('admin/admin.AddIntensiveCourse') }}</span>
                </a>
            @endcan
        </div>
                        ...
                        <tr>
                            ...
                            <td class="text-sm font-normal text-black34 fontp leading-normal">
                                <div class=" w-full mt-4">
                                    @can('change_status_intensive_courses')
                                        <form
                                            action="{{ route('admin.activeOrDisableCourse', ['courseType' => 'intenCourse', 'course_id' => $course->id]) }}"
                                            method="post">
                                            @csrf
                                            <select class="inputtext " name="status"
                            ...
                            <td class="text-sm font-normal leading-normal">
                                <div class="relative text-sm group justify-end gap-2 flex font-semibold ">
                                    ...
                                    @endcan
                                    @can('delete_intensive_courses')
                                        <a id="deleteCourse"
                                           href="{{ route('admin.deleteIntensiveCourse', ['course_id'=>$course->id]) }}"
