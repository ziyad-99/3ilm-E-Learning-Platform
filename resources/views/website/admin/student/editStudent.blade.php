...
@section('title')
    {{ trans('admin/editStudent.edit_student') }}
@endsection
    ...
    <div>
        <div class="w-full md:ps-24 md:px-10">
            <h4 class="font-bold text-black34 mx-auto my-5 text-2xl px-4 lg:leading-[120%] ">
                <span>{{ trans('admin/editStudent.students') }} / {{ trans('admin/editStudent.edit_student') }}</span>
            </h4>
            @if ($errors->any())
            ...
            @endif
            <form class="w-full" onsubmit="return validateForm()" action="{{ route('admin.updateStudentProfile') }}" method="post"
                ...
                <div class="  w-full bg-white rounded-2xl p-4  py-10 md:px-9">
                    <div class="w-full flex flex-wrap ">
                        <div class=" w-full md:w-7/12 ">
                            ...
                            <div class="w-full mt-10">
                                <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editStudent.first_name') }}</p>
                                <input class="inputtext fontp" type="text" name="firstName"
                                       placeholder="{{ trans('admin/editStudent.first_name') }}" required
                                       value="{{ $student->firstName }}"/>
                            </div>
                            <div class="w-full mt-5">
                                <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editStudent.last_name') }}</p>
                                <input class="inputtext fontp" name="lastName"
                                       placeholder="{{ trans('admin/editStudent.last_name') }}" required
                                       value="{{ $student->lastName }}"/>
                            </div>
                            <div class=" flex-wrap flex w-full mt-4">
                                <div class="w-full md:w-6/12 pe-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editStudent.email') }}</p>
                                    <input class="inputtext fontp" name="email" placeholder="" required
                                           value="{{ $student->email }}"/>
                                </div>
                                <div class=" w-full md:w-6/12 ps-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editStudent.phone') }}</p>
                                    <input id="phoneInput" class="inputtext fontp" name="phone"
                                           placeholder="{{ trans('admin/editStudent.phone') }}" required
                                           value="{{ $student->phone }}"
                            ...
                            <div class=" flex-wrap flex w-full mt-4">
                                <div class="w-full md:w-6/12 pe-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editStudent.password') }}</p>
                                    <input class="inputtext fontp" type="password" name="password"
                                           placeholder="**************"
                                    />
                                </div>
                                <div class=" w-full md:w-6/12 ps-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editStudent.confirmation_password') }}</p>
                            ...
                            <div class=" flex-wrap flex w-full mt-4">
                                <div class="w-full md:w-6/12 pe-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editStudent.state') }}</p>
                                    <select class="inputtext " name="state">
                                        <option value="{{ $student->state }}"
                                                selected>{{ trans('admin/admin.Select_aWilaya') }}</option>
                                ...
                                <div class=" w-full md:w-6/12 ps-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editStudent.address') }}</p>
                                    <input class="inputtext fontp" name="address"
                                           placeholder="{{ trans('admin/editStudent.address') }}" required
                                           value="{{ $student->address }}"/>
                                </div>
                            </div>

                            <div class=" w-full mt-4">
                                <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editStudent.gender') }}</p>
                                <select class="inputtext " name="gender">
                                    <option value="" disabled>{{ trans('admin/editStudent.select_gender') }}</option>
                                    <option
                                        value="male" {{ $student->gender === 'male' ? 'selected' : ''}}>{{ trans('admin/editStudent.male') }}
                                    </option>
                                    <option
                                        value="female" {{ $student->gender === 'female' ? 'selected' : ''}}>{{ trans('admin/editStudent.female') }}
                                    </option>
                                </select>
                            </div>

                            <div class=" flex-wrap flex w-full mt-4">
                                <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editStudent.status') }}</p>
                                <select class="inputtext " name="status" required>
                                    <option value="" selected
                                            disabled>{{ trans('admin/editStudent.select_status') }}</option>
                                    <option
                                        value="0" {{ $student->status === 0 ? 'selected' : ''}}>{{ trans('admin/editStudent.disable') }}</option>
                                    <option
                                        value="1" {{ $student->status === 1 ? 'selected' : ''}}>{{ trans('admin/editStudent.active') }}</option>
                                </select>
                            </div>

                            <div class="w-full mt-4">
                                <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editStudent.bio') }}</p>
                                <textarea
                                    ...
                                    rows="3"
                                    placeholder="{{ trans('admin/editStudent.bio') }}"
                    ...
                    <button type="submit"
                            class=" btn items-center hover:bg-orange3 transform hover:scale-105 ms-0 mt-5 flex px-10">
                        <span class=" text-white font-bold text-sm ">{{ trans('admin/editStudent.edit') }}</span>
                    </button>
                </div>
            </form>
        </div>
