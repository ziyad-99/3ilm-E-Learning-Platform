...
@section('title')
    {{ trans('admin/admin.studentPanel') }}
@endsection
    ...
    <div>
        <div class="w-full md:ps-24 md:px-10">
            <h4 class="font-bold text-black34 mx-auto my-5 text-2xl lg:leading-[120%] ">
                <span>{{ trans('admin/admin.studentPanel') }}</span>
            </h4>
                            ...
                            <div class="md:w-8/12 flex gap-2">
                                ...
                                <div class="md:w-2/12 gap-2" id="primarySchoolYears" style="display: none;">
                                    <select class="inputtext" name="primary_school_year">
                                        ...
                                        <option value="3ap">{{ trans('admin/admin.2ap') }}</option>
            ...
            @can('add_student')
                ...
                <span class="text-white font-bold text-sm">
                    {{ trans('admin/allStudents.add_student') }}
                </span>
                </a>
            @endcan
            ...
            <div id="ajax_search_result">
                <table class="table admin" datatable id="datatable-search-list">
                    ...
                    <tr class="bg-orange/30">
                        <th class="font-bold uppercase text-orange text-xxs opacity-70">{{ trans('admin/allStudents.image') }}</th>
                        <th class="font-bold uppercase text-orange text-xxs opacity-70">{{ trans('admin/allStudents.name') }}</th>
                        <th class="font-bold uppercase text-orange text-xxs opacity-70">{{ trans('admin/allStudents.phone_number') }}</th>
                        <th class="font-bold uppercase text-orange text-xxs opacity-70">{{ trans('admin/allStudents.wilaya') }}</th>
                        <th class="font-bold uppercase text-orange text-xxs opacity-70">{{ trans('admin/allStudents.status') }}</th>
                        ...
                        <tr>
                            ...
                            <td class="text-sm font-normal text-black34 fontp leading-normal">
                                ...
                                <div class=" w-full mt-4">
                                    @can('change_status_student')
                                        <form
                                            action="{{ route('admin.activeOrDisableStudent', ['student_id' => $student->id]) }}"
                                            method="post">
                                            @csrf
                                            <select class="inputtext " name="status" onchange="this.form.submit()">
                                                ...
                                                <option
                                                    value="0" {{ $student->status == 0 ? 'selected' : ''}}>
                            ...
                            <td class="text-sm font-normal leading-normal">
                                <div class="relative text-sm group justify-end gap-2 flex font-semibold ">
                                    ...
                                    @can('delete_student')
                                        <a id="deleteStudent" href="{{ route('admin.deleteStudent', ['student_id'=>$student->id]) }}"
    ...
    <script>
        $(document).ready(function () {
            $('form').submit(function (event) {
                ...
                $.ajax({
                    url: "{{ route('admin.ajaxFilterStudentByCourse') }}", // Get the form's action attribute
