...
@section('title')
    {{ trans('admin/editInstructor.edit_instructor') }}
@endsection
    ...
    <div>
        ...
        <div class="w-full md:ps-24 md:px-10">
            <h4 class="font-bold text-black34 mx-auto my-5 text-2xl px-4 lg:leading-[120%] ">
                <span>{{ trans('admin/editInstructor.instructors') }} / {{ trans('admin/editInstructor.edit_instructor') }}</span>
            </h4>
            ...
            @endif
            <form class="w-full" onsubmit="return validateForm()" action="{{ route('admin.updateInstructorProfile') }}"
                  method="post"
                ...
                <div class="  w-full bg-white rounded-2xl p-4  py-10 md:px-9">
                    <div class="w-full flex flex-wrap ">
                        <div class=" w-full md:w-7/12 ">
                            ...
                            <div class="w-full mt-10">
                                <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editInstructor.first_name') }}</p>
                                <input class="inputtext fontp" type="text" name="firstName"
                                       placeholder="{{ trans('admin/editInstructor.first_name') }}" required
                                       value="{{ $instructor->firstName }}"/>
                            </div>
                            <div class="w-full mt-5">
                                <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editInstructor.last_name') }}</p>
                                <input class="inputtext fontp" name="lastName"
                                       placeholder="{{ trans('admin/editInstructor.last_name') }}" required
                                       value="{{ $instructor->lastName }}"/>
                            </div>
                            <div class=" flex-wrap flex w-full mt-4">
                                <div class="w-full md:w-6/12 pe-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editInstructor.email') }}</p>
                                    <input class="inputtext fontp" name="email"
                                           placeholder="{{ trans('admin/editInstructor.email') }}" required
                                           value="{{ $instructor->email }}"/>
                                </div>
                                <div class=" w-full md:w-6/12 ps-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editInstructor.phone') }}</p>
                                    <input id="phoneInput" class="inputtext fontp" name="phone"
                                           placeholder="{{ trans('admin/editInstructor.phone') }}" required
                                           value="{{ $instructor->phone }}"
                                           onchange="hideError('phoneError')"/>
                                    @error('phone')
                                    <p id="phoneError" class="text-red-500 text-sm">{{ $message }}</p>
                                    @enderror
                                    <script>
                                        function validateForm() {
                                            var phoneInput = document.getElementById("phoneInput");
                                            if (phoneInput.value.length !== 10) {
                                                alert("{{ trans('website/website.Phone number must be exactly 10 characters long.') }}");
                            ...
                            <div class=" flex-wrap flex w-full mt-4">
                                <div class="w-full md:w-6/12 pe-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editInstructor.password') }}</p>
                                    <input class="inputtext fontp" type="password" name="password"
                                           placeholder="**************"
                                    />
                                </div>
                                <div class=" w-full md:w-6/12 ps-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editInstructor.confirmation_password') }}</p>
                            ...
                            <div class=" flex-wrap flex w-full mt-4">
                                <div class="w-full md:w-6/12 pe-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editInstructor.state') }}</p>
                                    <select class="inputtext " name="state">
                                        <option value="{{ $instructor->state }}"
                                                selected>{{ trans('admin/editInstructor.select_state') }}</option>
                                ...
                                <div class=" w-full md:w-6/12 ps-2">
                                    <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/addStudent.address') }}</p>
                                    <input class="inputtext fontp" name="address"
                                           placeholder="{{ trans('admin/addStudent.address') }}"
                                           required
                            ...
                            <div class=" w-full mt-4">
                                <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editInstructor.Gender') }}</p>
                                <select class="inputtext " name="gender">
                                    <option value="" disabled>{{ trans('admin/editInstructor.select_gender') }}</option>
                                    <option
                                        value="male" {{ $instructor->gender === 'male' ? 'selected' : ''}}>{{ trans('admin/editInstructor.male') }}
                                    </option>
                                    <option
                                        value="female" {{ $instructor->gender === 'female' ? 'selected' : ''}}>{{ trans('admin/editInstructor.female') }}
                                    </option>
                                </select>
                            </div>

                            <div class=" flex-wrap flex w-full mt-4">
                                <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editInstructor.status') }}</p>
                                <select class="inputtext " name="status" required>
                                    <option value="" selected
                                            disabled>{{ trans('admin/editInstructor.select_status') }}</option>
                                    <option
                                        value="0" {{ $instructor->status === 0 ? 'selected' : ''}}>{{ trans('admin/editInstructor.disable') }}</option>
                                    <option
                                        value="1" {{ $instructor->status === 1 ? 'selected' : ''}}>{{ trans('admin/editInstructor.active') }}</option>
                                </select>
                            </div>

                            <div class="w-full mt-4">
                                <p class="font-semibold text-black2 mb-2  text-sm ">{{ trans('admin/editInstructor.bio') }}</p>
                                <textarea
                                    ...
                                    rows="3"
                                    placeholder="{{ trans('admin/editInstructor.bio') }}"
                    ...
                    <button type="submit"
                            class=" btn items-center hover:bg-orange3 transform hover:scale-105 ms-0 mt-5 flex px-10">
                        <span class=" text-white font-bold text-sm ">{{ trans('admin/editInstructor.edit') }}</span>
                    </button>
                </div>
            </form>
        </div>
